...
namespace EuroJackpotMostOftenNumbers
{
    class Program
    {
        ...
        private static List<int> _last3Months;
        ...
        static void Main(string[] args)
        {
            ...
            while (drawDate < DateTime.Now)
            {
                ...
                if (externalResponse.winningNumbers != null)
                {
                    foreach (var winningNumber in externalResponse.winningNumbers)
                    {
                        ...
                        if((DateTime.Now.Year == drawDate.Year) && (DateTime.Now.Month - 2 <= drawDate.Month))
                            _last3Months.Add(winningNumber);
                    }
                }

                if (externalResponse.starNumbers != null)
                {
            ...
            {
                if (_last3Months.Where(a => a.Equals(item.Key)).ToList().Count() >= 3)
                    Console.ForegroundColor = ConsoleColor.Red;
                else if (_last3Months.Where(a => a.Equals(item.Key)).ToList().Count() == 2)
                    Console.ForegroundColor = ConsoleColor.Yellow;
                else if (_last3Months.Where(a => a.Equals(item.Key)).ToList().Count() == 1)
